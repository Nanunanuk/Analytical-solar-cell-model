...
%% For storing generation profiles set the parameter store_G=1
store_G=0;
...
AM15=[a(:,1) a(:,2)];
%% Basic constants
epsilon=11.8*8.854187817e-12;   %Permittivity of free space [C/Vm]
q=1.602E-19;                    %Elementary charge [Js]
k=1.38064852E-23;               %Boltzmann's constant [J/K]
Temp=300;                       %Cells temperature [K]
n_i=1e10;                       %Intrinsic carrier density [1/cm³]
h=6.62606957E-34;               %Planck's constant [Js]
c_0=299792458;                  %Vacuum speed of light [m/s]
%% Global parameter
...
for theta_0=[8];                    %Batch parameter: Angle of incidence [°]
    ...
    d_AR=86;                        %Thickness of AR-coating [nm]
    for W=[200]./1000000   ;        %Batch parameter: Thickness of solar cell [m]
        lambda=(300:10:1200)';      %Wavelength [nm]
            ...
            for h_f=[0.1];          % Batch parameter: Haze factor for front side 
                for r_db=[0.81];    % Batch parameter: Diffuse reflectivity at back side 
                    for r_df=[0.97];                % Batch parameter: Diffuse reflectivity at front side
                        ...
                        wf=180;  % Width of fingers [µm]                                           
                       
                        pbb=60.07;  % Pitch between busbars [mm] 
                        wbb=0;      % Width of busbars [mm] 
                        
                        Mbb=wbb/(pbb+wbb);              %Share of busbars
                        Mfi=pbb*wf/((pbb+wbb)*(pf+wf)); %Share of fingers
                        Mc=pbb*pf/((pbb+wbb)*(pf+wf));  %Share of cell surface
                        
                        N_D=1e20;   %Density of donnor atoms [1/cm³]
                        N_A=1e17;   %Density of acceptor atoms [1/cm³]
                        
                        V_0= Temp*k/q*log(N_A*N_D/n_i^2); %
                        
                        for W_e=[0.3] /1e6;                                 %Batch parameter: Width of emitter [m]
                            W_scr=sqrt(2*epsilon/q*V_0*(1/N_A+1/N_D))/1000; %Width of space charge region [m]
                            W_b=W-W_e-W_scr;                                %Width of base [m]                     
                            
                            L_b_count=1;
                            for L_b=[200]   /1e6;           %Batch parameter: Base diffusion length
                            L_b_contour(L_b_count)=L_b*1e6; %Variable for contour plot
                            L_b_count=L_b_count+1;          
                            
                                for L_e=[12]     /1e6;          %Batch parameter: Diffusion lenght of emitter
                                    for S_f=[1E4] /100;         %Batch parameter: Front surface recombination velocity 
                                        
                                        S_r_count=1;
                                        for S_r=[800] /100;                 %Batch parameter: Rear surface recombination velocity
                                            S_r_contour(S_r_count)=S_r*100; %Variable for contour plot
                                            S_r_count=S_r_count+1;
                                
                                            D_p=1.5   /10000;   %Diffusion coefficient of emitter
                                            D_n=29  /10000;     %Diffusion coefficient of base
                                            
                                            alpha=4*pi*interp1(Si(:,1),Si(:,3),lambda)./(lambda(:,1)/1e9);  %Absorption coefficient
                                            phi=interp1(AM15(:,1),AM15(:,2),lambda)/(h*c_0*1E9).*lambda;    %Number of photons in AM1.5 spectrum    
                                            ...
                                            for pol=['s' 'p'];
                                                ...
                                                for n=1:length(lambda)
                                                    ...
                                                    eta_forw_e_a(n,:)=alpha(n,:)/cos(theta_t(n,:))*L_e/...
                                                        (((alpha(n,:)/cos(theta_t(n,:)))^2*L_e^2-1)*(1-exp(-alpha(n,:)/cos(theta_t(n,:))*W_e)))...
                                                        *(alpha(n,:)/cos(theta_t(n,:))*L_e...
                                                        -(sinh(W_e/L_e)+S_f*L_e/D_p*cosh(W_e/L_e)-(S_f*L_e/D_p-alpha(n,:)/cos(theta_t(n,:))*L_e)...
                                                        *exp(-alpha(n,:)/cos(theta_t(n,:))*W_e))...
                                                        /(cosh(W_e/L_e)+S_f*L_e/D_p*sinh(W_e/L_e)));
                                                    eta_back_e_b(n,:)=alpha(n,:)/cos(theta_t2(n,:))*L_e/...
                                                        (((alpha(n,:)/cos(theta_t2(n,:)))^2*L_e^2-1)*(1-exp(-alpha(n,:)/cos(theta_t2(n,:))*W_e)))...
                                                        *(-alpha(n,:)/cos(theta_t2(n,:))*L_e*exp(-alpha(n,:)/cos(theta_t2(n,:))*W_e)-...
                                                        ((sinh(W_e/L_e)+S_f*L_e/D_p*cosh(W_e/L_e))*exp(-alpha(n,:)/cos(theta_t2(n,:))*W_e)...
                                                        -(S_f*L_e/D_p+alpha(n,:)/cos(theta_t2(n,:))*L_e))...
                                                        /(cosh(W_e/L_e)+S_f*L_e/D_p*sinh(W_e/L_e)));
                                                    eta_forw_e_c(n,:)=alpha(n,:)/cos(theta_diff(n,:))*L_e/...
                                                        (((alpha(n,:)/cos(theta_diff(n,:)))^2*L_e^2-1)*(1-exp(-alpha(n,:)/cos(theta_diff(n,:))*W_e)))...
                                                        *(alpha(n,:)/cos(theta_diff(n,:))*L_e-...
                                                        (sinh(W_e/L_e)+S_f*L_e/D_p*cosh(W_e/L_e)-(S_f*L_e/D_p-alpha(n,:)/cos(theta_diff(n,:))*L_e)...
                                                        *exp(-alpha(n,:)/cos(theta_diff(n,:))*W_e))...
                                                        /(cosh(W_e/L_e)+S_f*L_e/D_p*sinh(W_e/L_e)));
                                                    eta_back_e_d(n,:)=alpha(n,:)/cos(theta_diff(n,:))*L_e/...
                                                        (((alpha(n,:)/cos(theta_diff(n,:)))^2*L_e^2-1)*(1-exp(-alpha(n,:)/cos(theta_diff(n,:))*W_e)))...
                                                        *(-alpha(n,:)/cos(theta_diff(n,:))*L_e*exp(-alpha(n,:)/cos(theta_diff(n,:))*W_e)-...
                                                        ((sinh(W_e/L_e)+S_f*L_e/D_p*cosh(W_e/L_e))*exp(-alpha(n,:)/cos(theta_diff(n,:))*W_e)...
                                                        -(S_f*L_e/D_p+alpha(n,:)/cos(theta_diff(n,:))*L_e))...
                                                        /(cosh(W_e/L_e)+S_f*L_e/D_p*sinh(W_e/L_e)));
                                                    
                                                    %% Spectrum of the collection probability inside the base
                                                    eta_forw_b_a(n,:)=alpha(n,:)/cos(theta_t(n,:))*L_b/...
                                                        (((alpha(n,:)/cos(theta_t(n,:)))^2*L_b^2-1)*(1-exp(-alpha(n,:)/cos(theta_t(n,:))*W_b)))...
                                                        *(alpha(n,:)/cos(theta_t(n,:))*L_b-...
                                                        (sinh(W_b/L_b)+S_r*L_b/D_n*cosh(W_b/L_b)-(S_r*L_b/D_n-alpha(n,:)/cos(theta_t(n,:))*L_b)...
                                                        *exp(-alpha(n,:)/cos(theta_t(n,:))*W_b))...
                                                        /(cosh(W_b/L_b)+S_r*L_b/D_n*sinh(W_b/L_b)));
                                                    eta_back_b_b(n,:)=alpha(n,:)/cos(theta_t2(n,:))*L_b/...
                                                        (((alpha(n,:)/cos(theta_t2(n,:)))^2*L_b^2-1)*(1-exp(-alpha(n,:)/cos(theta_t2(n,:))*W_b)))...
                                                        *(-alpha(n,:)/cos(theta_t2(n,:))*L_b*exp(-alpha(n,:)/cos(theta_t2(n,:))*W_b)-...
                                                        ((sinh(W_b/L_b)+S_r*L_b/D_n*cosh(W_b/L_b))*exp(-alpha(n,:)/cos(theta_t2(n,:))*W_b)...
                                                        -(S_r*L_b/D_n+alpha(n,:)/cos(theta_t2(n,:))*L_b))...
                                                        /(cosh(W_b/L_b)+S_r*L_b/D_n*sinh(W_b/L_b)));
                                                    eta_forw_b_c(n,:)=alpha(n,:)/cos(theta_diff(n,:))*L_b/...
                                                        (((alpha(n,:)/cos(theta_diff(n,:)))^2*L_b^2-1)*(1-exp(-alpha(n,:)/cos(theta_diff(n,:))*W_b)))...
                                                        *(alpha(n,:)/cos(theta_diff(n,:))*L_b-...
                                                        (sinh(W_b/L_b)+S_r*L_b/D_n*cosh(W_b/L_b)-(S_r*L_b/D_n-alpha(n,:)/cos(theta_diff(n,:))*L_b)...
                                                        *exp(-alpha(n,:)/cos(theta_diff(n,:))*W_b))...
                                                        /(cosh(W_b/L_b)+S_r*L_b/D_n*sinh(W_b/L_b)));
                                                    eta_back_b_d(n,:)=alpha(n,:)/cos(theta_diff(n,:))*L_b/...
                                                        (((alpha(n,:)/cos(theta_diff(n,:)))^2*L_b^2-1)*(1-exp(-alpha(n,:)/cos(theta_diff(n,:))*W_b)))...
                                                        *(-alpha(n,:)/cos(theta_diff(n,:))*L_b*exp(-alpha(n,:)/cos(theta_diff(n,:))*W_b)-...
                                                        ((sinh(W_b/L_b)+S_r*L_b/D_n*cosh(W_b/L_b))*exp(-alpha(n,:)/cos(theta_diff(n,:))*W_b)...
                                                        -(S_r*L_b/D_n+alpha(n,:)/cos(theta_diff(n,:))*L_b))...
                                                        /(cosh(W_b/L_b)+S_r*L_b/D_n*sinh(W_b/L_b)));
                                            ...
                                            name_array(batch,:)={['theta: ' num2str(theta_0)] [', W: ' num2str(W)]...
                                                ...
                                                [', L_n: ' num2str(L_b)] [', L_p: ' num2str(L_e)]...
